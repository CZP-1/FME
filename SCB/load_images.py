    ...
    if(dataset_name == 'CASME_sq'):
        for i, dir_sub in enumerate(natsort.natsorted(glob.glob(dataset_name + "/rawpic_crop/*"))):
          ...
          for dir_sub_vid in natsort.natsorted(glob.glob(dir_sub + "/*")):
            subjectsVideos[-1].append(dir_sub_vid.split('/')[-1]) # Ex:'CASME_sq/rawpic_aligned/s15/15_0101disgustingteeth' -> '0101' 
            image = []
        ...
        # for i, dir_sub in enumerate(natsort.natsorted(glob.glob(dataset_name + "/SAMM_longvideos_crop/*"))):
        #   #print(dir_sub)
        #   print('Subject: ' + dir_sub.split('/')[-1])
        #   subjects.append(dir_sub.split('/')[-1])
        #   subjectsVideos.append([])
        #   for dir_sub_vid in natsort.natsorted(glob.glob(dir_sub + "/*")):
        #     subjectsVideos[-1].append(dir_sub_vid.split('/')[-1]) 
        #     image = []
        #     print(dir_sub_vid)
